...
func _test_public_read_only_api() -> void:
	...
	_ok("main.gd creates the FormalFlowRouter (P4-03)", main_script_text.contains("FormalFlowRouterScript.new()"))
	_ok("formal continue check uses read_progress (in FormalFlowRouter)", _read_text("res://scripts/controllers/formal_flow_router.gd").contains("TrainingManagerScript.read_progress()"))
