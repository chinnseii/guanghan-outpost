...
## Verifies the replaced title-screen background (real image instead of the
## hand-drawn placeholder). Run only against an isolated sandbox project
...
func _run() -> void:
	...
	await _capture("01_main_menu_real_background.png")

	root.size = Vector2i(1600, 900)
	await process_frame
	await create_timer(0.2).timeout
	await _capture("02_main_menu_1600x900.png")
