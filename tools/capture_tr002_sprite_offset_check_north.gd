...
func _run() -> void:
	...
	player.position = scene.call("_room_point", Vector2(terminal_blocker.get_center().x, terminal_blocker.position.y - 90.0))
	scene.call("_sync_player_visual")
	await process_frame
	await _walk_to(player, scene, Vector2(terminal_blocker.get_center().x, terminal_blocker.end.y + 10.0))
	await process_frame
	await process_frame

	var texture := root.get_texture()
	var image := texture.get_image()
	var err := image.save_png("%s/15_sprite_offset_fix_terminal_north.png" % OUT_DIR)
