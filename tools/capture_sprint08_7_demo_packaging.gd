...
const OUT_DIR := "res://docs/screenshots/sprint08_7_demo_packaging"
...
func _initialize() -> void:
	print("capture_sprint08_7_demo_packaging start")
...
func _run() -> void:
	...
	await _capture("01_main_menu_v05.png")

	_write_application_profile("review", "审核中", true)
	await _load_and_wait(MAIN_SCENE, null)
	if scene_instance.has_method("_show_new_game_confirmation"):
		scene_instance.call("_show_new_game_confirmation")
	await create_timer(0.25).timeout
	await _capture("02_new_game_confirmation_if_save_exists.png")

	_clear_demo_progress()
	await _load_and_wait(MAIN_SCENE, null)
	await _capture("03_continue_no_save_or_continue_available.png")

	_write_training_progress({"TrainingStarted": true, "CurrentTrainingModule": "suit_control"})
	await _load_and_wait(MAIN_SCENE, null)
	if scene_instance.has_method("_toggle_dev_menu"):
		scene_instance.call("_toggle_dev_menu")
	await create_timer(0.25).timeout
	await _capture("04_dev_menu_separated.png")

	_clear_demo_progress()
	await _load_and_wait(MAIN_SCENE, null)
	await _capture("05_input_hint.png")

	_clear_demo_progress()
	_write_application_profile("identity", "待提交", false)
	await _load_and_wait(APPLICATION_SCENE, null)
	await _capture("06_application_start_from_new_game.png")

	_write_application_profile("training_start", "训练序列中", true)
	_write_training_progress({"TrainingStarted": true, "CurrentTrainingModule": "suit_control"})
	await _load_and_wait(TRAINING_START_SCENE, null)
	await _capture("07_training_reached_from_normal_flow.png")
	...
	await _capture("08_mission_assignment_reached_from_training.png")
	...
	await _capture("09_lunar_arrival_reached_from_assignment.png")
	...
	await _capture("10_old_base_reached_from_arrival.png")

	_write_base_state(_base_state({"BaseEntered": false}))
	await _load_and_wait(AIRLOCK_SCENE, null)
	await create_timer(0.75).timeout
	await _capture("11_day02_reached_from_old_base.png")

	_write_base_state(_day02_state({"Day02ReportSent": true, "ArchiveEntry_Day02Report": true}))
	await _load_and_wait(OLD_BASE_SCENE, Vector2(1120, 620))
	await _capture("12_week_one_reached_from_day02.png")
	...
	await _capture("13_first_week_end_reached_from_day07.png")
	...
	await _capture("14_phase02_placeholder_demo_end.png")

	await _load_and_wait(MAIN_SCENE, null)
	await _capture("15_return_to_main_menu_from_demo_end.png")

	print("capture_sprint08_7_demo_packaging done")
	quit()
