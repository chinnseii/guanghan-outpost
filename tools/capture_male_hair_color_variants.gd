...
## Verifies the 2 newly registered male hair-color combos (blonde/auburn
## buzzcut) render correctly via the real set_character_appearance() API.
## Isolated player.tscn instantiate, no save file interaction.
const OUT_DIR := "res://docs/screenshots/male_hair_color_variants"
...
func _run() -> void:
	...
	var combos := [
		["male", "medium", "blonde", "buzz"],
		["male", "medium", "auburn", "buzz"],
	]
	for combo in combos:
