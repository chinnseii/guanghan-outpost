...
## 当前版本：V0.18 搬运、机器人队列与舱外风险

当前重点是把出舱任务做得更有重量：补给舱不能一键拿完，大件补给需要分批搬运；机器人充电桩支持任务队列；舱外加入低氧、太阳风暴、微陨石和月尘维护成本。
...
3. 舱外风险事件：
   ...
   - 月尘污染导致返舱维护成本上升
4. 正式视听资产替换：
   - 将代码绘制的像素风设施替换为独立 sprite
   - 添加正式音效和环境氛围
   - 给气闸、控制台、机器人充电桩加入更明确动画
5. 工程结构整理：
   - 把主脚本继续拆成 Resource 数据表和 Manager 节点
   - 为后续长期迭代降低复杂度
