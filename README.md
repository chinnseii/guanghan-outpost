...
## 当前版本：V0.7 TileMap 月面地形

当前重点是把原先脚本绘制的月面网格替换成 Godot 正式 TileMapLayer，为后续地形、碰撞、道路和素材化打基础。
...
- 保存/读取系统
- 存档路径：`user://guanghan_outpost_save.json`
- 可保存日期、资源、玩家位置、模块、作物、漏气、采集点、补给订单和日志
...
1. 加入更完整的气闸、舱压和外出氧气消耗。
2. 加入月壤提氧、冰矿处理、温室湿度和二氧化碳。
3. 加入嫦娥样本数据库、鹊桥中继、玉兔机器人升级线。
4. 加入多个存档槽和主菜单。
5. 把模块、玩家和采集点拆成独立场景与正式素材。
