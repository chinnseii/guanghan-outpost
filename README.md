...
## 当前版本：V0.20 新手任务卡、实时机器人与音效入口

当前重点是减少“我现在该干嘛”的迷茫，并让机器人真正进入实时玩法：左上角目标改成今日任务卡，地图会用箭头指向控制台、气闸、储物柜、温室和补给区；机器人任务从跨天结算改为实时队列，机器人会移动到目标点后再完成采样、巡检或搬运；音效入口和机器人任务逻辑继续从 `main.gd` 拆出。
