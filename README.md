...
## 当前版本：V0.29 机器人、任务体验与正式素材第三轮

当前重点是把“自动化劳动力、任务反馈、舱外识别度”继续做扎实：三类机器人已经拥有不同外观和状态灯；任务完成会出现短暂屏幕反馈和地图标记；当前目标、出舱任务、长期目标合并为目标栈，并支持 `T` 键自动居中追踪；机器人、补给舱和月面采集点接入第三批独立 sprite，并新增统一 sprite 尺寸规范。
