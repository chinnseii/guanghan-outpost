...
## 当前版本：V0.19 可见机器人、独立页面与舱内交互

当前重点是让玩家更容易看懂“基地里有什么、自己正在操作什么”：机器人已经作为实体出现在地图上并会向任务目标移动；控制台、储物柜、补给舱有独立信息页面；舱内家具加入碰撞与明确交互高亮；存档和提示音逻辑开始从 `main.gd` 拆到 Manager 脚本。
...
1. 新手引导强化：
   - 第一次进入游戏时用箭头或高亮指向控制台、气闸、储物柜和温室
   - 把“今日该做什么”变成更明确的任务卡
2. 机器人玩法深化：
   - 让机器人按队列实时工作，而不是主要在跨天结算
   - 给采样、巡检、搬运增加可见路径线和任务完成反馈
3. 正式素材第二轮：
   - 将当前代码绘制家具逐步替换为独立 sprite 资源
   - 添加环境氛围音、气闸音效、脚步/工具音效
4. 工程结构整理：
   - 继续把资源表、任务系统、机器人系统拆出 `main.gd`
   - 为长期迭代降低复杂度
