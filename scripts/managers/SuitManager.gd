...
func remove_suit_to_service_station_training() -> bool:
	...
	if training_time_manager == null or not training_time_manager.has_method("advance_training_time"):
		return false
	training_time_manager.call("advance_training_time", remove_to_station_time_minutes, "training_return_spacesuit")
	is_suit_worn = false
	suit_storage_state = "servicing"
	suit_oxygen = max(suit_oxygen, suit_oxygen_capacity)
...
## Snapshot for UI code that wants all status readouts in one call
## (oxygen/power/speed) instead of reading fields individually -- used by
## the 宇航服整备室 training-room's status panel, but generic enough for any
## future suit UI.
...
func panel_status_text() -> String:
	var lines: Array[String] = [
		...
		"行动速度倍率：%.2f（%s）" % [
			suit_speed_multiplier, get_suit_level_name(),
		],
	]
	if suit_storage_state == "servicing":
		var missing_oxygen: float = suit_oxygen_capacity - suit_oxygen
