...
const BaseHudPanelPresenterScript := preload("res://scripts/controllers/base_hud_panel_presenter.gd")

var _hud: RefCounted = null
...
func _setup_ui() -> void:
	_hud = BaseHudPanelPresenterScript.new()
	_hud.build_ui(self)
	# Re-expose the presenter-built nodes to the scene vars that the flow/HUD code updates,
	# so those update sites stay unchanged (P4-04 moved construction only, not the updates).
	hud_label = _hud.hud_label
	message_label = _hud.message_label
	prompt_label = _hud.prompt_label
	ai_label = _hud.ai_label
	time_hud_panel = _hud.time_hud_panel
	time_hud_label = _hud.time_hud_label
	fade_rect = _hud.fade_rect
	interaction_panel = _hud.interaction_panel
	interaction_label = _hud.interaction_label
	interaction_bar = _hud.interaction_bar
	# Plant-diagnosis modal stays in the scene (its action buttons drive gameplay).
	_setup_plant_diagnosis_ui(_hud.ui_root)
