...
- **批次顺序**：P3-02 ✅ → P3-02R ✅ → **P3-03（需用户先确认方案 C + 兼容策略）**：P3-03a 恢复一致性缺口修复（Power/Suit mirror 同步、restore-complete、read/restore API 边界）→ P3-03b Full Save Orchestrator 正式化（+schema_version）→ P3-03c Manager 自存降级 → P3-03d checkpoint 越域裁剪 → P3-04（职责重叠清洗 + Inventory/Backpack 字段核实 + Door 正式基地接入）→ P3-05（Legacy 隔离，含同名局部节点）→ P3-06（回归收口）。
- **具体任务**（每个系统独立 PR，串行）：
