...
- **Application UI modernization line**：AUI-DOC-01 registered the approved AUI-03-01 reference/spec。**AUI-03-01 状态：`DONE`，`VISUAL_PASS_WITH_MINOR_ADJUSTMENTS`，已 push**（Owner transfer Codex → Claude Code；理由：Codex 的实现未达批准视觉标准，User 指示重建 `_show_identity()` 视觉节点树，保留全部业务逻辑/绑定/存档行为不变）。首次验收通过后，User 在真实（非强制分辨率）运行下发现底部操作栏完全不可见——根因是 `_show_identity()` 手动禁用了滚动、而项目真实默认视口是 1600×900（非 1920×1080），零余量布局在该尺寸下溢出把底栏顶出视口。核心修复：在场景内部（仅 `application_flow_scene.gd`，不改 `project.godot` 全局设置、不影响其他场景）实现唯一权威缩放方案——整页装进固定 1920×1080 的 `aui_canvas`，按 `min(宽/1920, 高/1080)` 统一等比缩放并居中（letterbox/pillarbox），在 resize 信号与每帧兜底两处重算；彻底移除了原先作为退路的 ScrollContainer。已验证 1920×1080（无滚动条、底栏完整）、1600×900（精确 83.33% 等比缩放）、1440×900 非 16:9（像素级测量确认上下留白对称、不拉伸不裁切）。全局 stretch mode 改动已与 User 讨论并明确列为独立任务、本轮不做。同轮还统一了三个可编辑控件的 16px 左内边距（根因是 OptionButton 样式完全没设 `content_margin`）、Mission Brief 连接线换成真实线条控件、Header 三段稳定分区、若干最小辅助字号上调、下一步箭头缩小约 20%、只读锁图标进一步降低亮度、下拉选项内边距调整。Godot 4.7 headless parse EXIT 0；既有 29-check 测试 `tests/aui_03_01_basic_information_test.gd` 全程原样通过；所有截图验证均在隔离沙盒项目副本中进行（复制项目 + 更改 `project.godot` 的 `config/name` 以获得独立 `user://` 路径），每轮前后均核对真实项目存档 SHA-256，内容与 User 确认过的自测状态保持一致。已知遗留：下拉弹出菜单是原生独立窗口层，非 `aui_canvas` 子节点，非 1.0 缩放下弹窗文字可能不会等比缩小，本轮未处理。02/03/04 页面与其他系统未改动；未修改保存逻辑/schema；所有五个 Skill 仍为 `TRIAL`；User 明确授权本轮 push（"通过，可以push"），未 tag。
