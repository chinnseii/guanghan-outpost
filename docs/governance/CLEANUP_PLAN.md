...
- **Application UI modernization line**：AUI-DOC-01 registered the approved AUI-03-01 reference/spec. **AUI-03-01 状态：`DONE`**（Owner transfer Codex → Claude Code；理由：Codex 的实现未达批准视觉标准，User 指示重建 `_show_identity()` 视觉节点树，保留全部业务逻辑/绑定/存档行为不变）。经四轮真实 1920×1080 截图迭代（底部操作栏四区块+状态圆环徽标、Mission Brief 连接线改为真实线条控件而非文字符号、Header 图标文字垂直对齐、下一步按钮箭头顺序）后 User 给出最终验收，同时覆盖视觉与工程正确性（本任务的 Owner/Reviewer 身份冲突由 User 直接终审解决，不再需要独立工程 Reviewer）。Godot 4.7 headless parse EXIT 0；既有 29-check 测试 `tests/aui_03_01_basic_information_test.gd` 原样通过；所有截图验证均在隔离沙盒项目副本中进行（复制项目 + 更改 `project.godot` 的 `config/name` 以获得独立 `user://` 路径），每轮前后均核对真实项目存档 SHA-256 不变。记录一次教训：早期一次截图尝试误判 `--user-data-dir` 会隔离而未生效，短暂触及真实存档（无内容损失，哈希前后一致），后改用 Codex 提供的正确沙盒方案解决。02/03/04 页面与其他系统未改动；未修改保存逻辑/schema；所有五个 Skill 仍为 `TRIAL`；未 push、未 tag。
