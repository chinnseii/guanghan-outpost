...
- **P4-04 ✅ DONE（2026-07-12，调整原计划）**：原计划 P4-04=sandbox 存档聚合，**延期**（legacy/dev、20+ 共享字段、收益低）；改为抽 `sprint06_base_scene.gd` 的 **`BaseHudPanelPresenter`**（`scripts/controllers/base_hud_panel_presenter.gd`，263 行，RefCounted，非 Autoload）。移出全部 HUD/状态面板 UI 构建 + 8 面板 toggle + `refresh_open_panels`；场景 `_setup_ui` 用 **re-expose** 把 flow 更新的 label 节点回指自身 var（更新点全不变=安全）；plant-diagnosis（按钮驱动玩法）与 save/flow/导航留场景。`sprint06_base_scene.gd` **2556→2331（净 −225）**。新增 `p4_04` 35/35，Phase3+P4-02/03 全绿，真实存档 SHA 复原。**下一步 P4-05 sprint06 导航/日程流程控制器**（或回头做延期的 sandbox 存档聚合）。
