...
## P4-05 Completion Note (2026-07-12) — P4-05A interface-prep scope

- **DONE (modest, by design)**: extracted the safely-separable navigation **computation** from `sprint06_base_scene.gd` into a stateless `scripts/controllers/base_navigation_controller.gd` (49 lines, `class_name`/RefCounted, non-Autoload). `sprint06_base_scene.gd` **2331 → 2308 (net −24)**.
- **Navigation responsibility map**: `_update_target` (target proximity), `_near`, `_current_terrain_type` = **pure computation → MOVED**. `_move_player`/`_ensure_player_controller` = movement main loop (advances time) → KEEP. `_world_left_limit` = reads schedule (`_is_week_routine_active`/`_is_day02_active`) → FLOW_COUPLED, KEEP. `_transition_to` = 12 flow callers + input/sequence flags + fade UI → FLOW_COUPLED, KEEP. `_interaction_target_rect` = dense day/schedule/state logic → FLOW_COUPLED, KEEP. `current_target` = read by ~40 flow sites → stays a scene field.
- **Moved**: `_current_terrain_type` → `terrain_type_for(scene_kind)`; `_near` → `is_near(pos, rect)`; `_update_target` body → `compute_current_target(pos, scene_kind, interior_targets, greenhouse_targets)`. Scene keeps thin `_update_target`/`_current_terrain_type` delegators (call sites unchanged); target maps stay in the scene (also used by prop setup + `_interaction_target_rect`) and are passed as args. Removed the now-dead `InteractionAreaScript` preload from the scene.
- **Controller state boundary**: stateless (no fields); pure functions only; no Manager write / save / scene change / task/schedule state / HUD-presenter dependency (test-asserted).
- **Call direction**: scene → `_nav.compute_*` (pure) → scene assigns `current_target` and drives all flow itself. Controller never calls back into the scene.
- **Behavior unchanged**: `compute_current_target` mirrors the old `_update_target` exactly (same iteration order, first-match-wins, same 96px sleep radius) — characterization test locks this in.
- **Why P4-05A (not a large extraction)**: sprint06 navigation is largely flow-coupled; a bigger move would pull in `current_target` consumers, transitions, and day/schedule logic, violating the boundaries. Per §9/§12 the safe amount (<80 lines) is interface-prep, not a failure. This establishes a testable navigation interface for future work.
- **Test**: `tests/p4_05_base_navigation_controller_test.gd` (30/30) — terrain mapping, proximity, target computation (interior/greenhouse/sleep/first-match/unknown), and static boundary (controller nav-only; scene keeps transition/interaction-rect/Full Save/current_target flow/movement).
...
## P4-07A Note (2026-07-12) — training large-script UI audit (no code moved)

- **DONE (audit + characterization only)**: mapped `training_module_scene.gd` (3417) and `training_base_map.gd` (2255) responsibilities/shared-state/SceneTree coupling → `docs/governance/P4_07A_TRAINING_LARGE_SCRIPT_AUDIT.md`. **Zero production code moved.**
- **Key findings**: both scripts build their training UI **dynamically** (`add_child` 60/54, **0** `$Node` hardcoded paths, **0** tween) → UI-presenter extraction needs **no `.tscn` change** (P4-04 pattern applies). The UI is flow-wired (footer→`set_current_module` checkpoint, suit-confirm→`_complete_step`, modal visibility reads `completed`/step). No P0/P1: training progress is canonical in `training_progress.json` (TrainingManager, P3-defined); scene writes only at checkpoint points; UI node refs are DERIVED_UI, not a second gameplay-state copy. Neither script touches formal Full Save.
- **Characterization**: `tests/p4_07a_training_large_script_audit_test.gd` (30/30, source-analysis, **no training/base-scene boot**) locks big-script facts, UI-vs-flow-vs-mutation-vs-async boundary samples, checkpoint localization (`set_current_module`/`mark_module_completed` stay in scene, not in builders), and SceneTree boundary (dynamic UI, no tween).
