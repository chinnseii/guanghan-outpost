...
# P3-05 Legacy Runtime Isolation (2026-07-12)

- Legacy runtime isolation completed: the sandbox (`main.gd`) and arrival prototype (`arrival_landing_scene.gd`) runtime paths are isolated from formal autoloads, Full Save, and the formal continue flow.
- Same-name risk eliminated: the only true collision was the local `TimeManager` node vs `/root/TimeManager`. Local sandbox/arrival manager node names are renamed with `Sandbox…` / `ArrivalPrototype…` prefixes. Safe because both files access these managers only through member variables — repository has zero `get_node("TimeManager")` / `$TimeManager` / `%TimeManager` name-path lookups. Formal autoload access stays on `/root/…`.
- Legacy save isolation is confirmed and documented (structurally already true from P3-03b/c/d): distinct namespaces — Full Save `full_save.json`, Training `training_progress.json`, sandbox `slot_N.json`, arrival `arrival_prototype_save.json`, legacy sprint06 `sprint06_progress.json` (read-only best-effort). `FullSaveOrchestrator.restore_full_save()` rejects `legacy_source`; it never reads arrival/sandbox files; sandbox/arrival saves never write `full_save.json` or touch `/root/*Manager`.
- Formal entry isolation confirmed: `_continue_mission` routes formal continue through `FullSaveOrchestrator.restore_full_save()`, training through `TrainingManager`, with a clearly-commented last-resort legacy sandbox-slot fallback that the formal flow does not depend on. Sandbox/arrival entry points are Dev/sandbox-panel only, not reachable from the formal Continue/New-Game menu path.
- Remaining risk / not done in P3-05: legacy files are NOT deleted; DoorStateManager formal-base integration NOT added; no schema/gameplay/`project.godot` change; `main.gd` large-script split deferred to Phase 4; `interaction_detector` orphan and `BaseInterior_Test` entry remain UNKNOWN.
- Verification: Godot editor parse EXIT 0; Godot headless smoke EXIT 0; P3-05 32/32; P3-03a 39/39; P3-03b 50/50; P3-03c 33/33; P3-03d 25/25; P3-04 33/33; real `user://saves/` SHA-256 identical before/after (test suites self-restore).

# P3-06 Phase 3 Regression & Closure (2026-07-12)

- Full Phase 3 regression re-run under takeover: P3-03a 39/39, P3-03b 50/50, P3-03c 33/33, P3-03d 25/25, P3-04 33/33, P3-05 **36/36** (P3-05 extended with cinematic checks); Godot editor/smoke EXIT 0; real `user://saves/` SHA-256 identical before/after; no residue.
- **Regression gap found & fixed (minimal)**: static scan `rg 'name = "TimeManager"'` surfaced a residual legacy collision in `scripts/arrival/arrival_cinematic_scene.gd` (local `game_state_manager`/`time_manager` nodes named "GameStateManager"/"TimeManager") that P3-05 did not cover. Renamed to `ArrivalCinematic{GameState,Time}Manager` (member-var access only, safe). Repository-wide `name = "TimeManager"/"GameStateManager"` count is now **0**.
- Static boundary confirmations: no external script calls `TrainingManager._read_progress_data()` or `load_progress()` in query context; formal restore entries are `FullSaveOrchestrator.restore_full_save()` only (`main.gd`, `sprint06_base_scene.gd`); manager `tree.root.get_node_or_null("TimeManager")` calls all resolve the FORMAL `/root/TimeManager` autoload (correct, not a collision).
- Phase 3 is **COMPLETE / CLOSED**. See `PHASE_3_CLOSURE_REPORT.md` for the consolidated final state, closed risks, and deferred work. No P0. Deferred (not regressions): DoorState formal-base integration, `main.gd`/`sprint06_base_scene.gd` split (Phase 4), legacy deletion, `interaction_detector`/`BaseInterior_Test` UNKNOWN.
